# PaperPilot — Technical Reference (Core Logic & Deep Dive)

This document details the internal mechanics of the PaperPilot evaluation engine, including RAG retrieval, embedding mathematics, and evaluation heuristics.

---

## 🔢 1. Embedding & Vector Logic

PaperPilot uses a custom, deterministic **Hash-based Embedding** strategy for zero-cost semantic representation.
...
- `syllabus`: Stores `chunks` (text strings) and `embeddings` (List of floats) for semantic search.
- `evaluations`: Stores scoring breakdowns, `similarity_score`, `retrieved_chunks`, and overall feedback.
- `teachers`: Stores hashed passwords, teacher info, and configurations.
- `students`: Central repository for student profiles and progress tracking.
- `classes`: Configures and groups students/syllabi by specific school classes.
- `sections`: Manages different sections within a class.
- `feedback_logs`: Logs teacher adjustments (AI vs. validated scores) to power the Adaptive Learning engine.
- `answer_scripts`: Stores details of uploaded student answer scripts (text, page count, first-page thumbnail).
