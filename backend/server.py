...
async def lifespan(app: FastAPI):
    ...
    if not GROQ_API_KEY:
        logger.warning(
            "GROQ_API_KEY not set — OCR cleanup and answer evaluation will fail. "
            "Set it in backend/.env (get a key at console.groq.com)."
        )
...
class LargeUploadMiddleware(BaseHTTPMiddleware):
    """Middleware to handle large file uploads (up to 100MB)"""
    async def dispatch(self, request: StarletteRequest, call_next):
        # Increase max body size for upload endpoints
        if request.url.path in ["/api/syllabus/upload-file", "/api/answer/ocr", "/api/students/bulk"]:
            # Set a large max body size (100MB = 100 * 1024 * 1024 bytes)
            request.scope["_max_body_size"] = 100 * 1024 * 1024
...
class AnswerScript(BaseModel):
    ...
    subject: str
    topic: Optional[str] = None
    image_data: Optional[str] = None # First page for thumbnail (preview)
    page_count: int = 1  # Number of pages in the original document
    ocr_text: str
...
@api_router.get("/")
async def root():
    return {"message": "PaperPilot API - Subjective Answer Evaluation System"}
...
async def evaluate_answer_script(answer_data: dict):
    ...
    try:
        ...
        # Get page count from request (default to 1 if not provided)
        page_count = answer_data.get('page_count', 1)
        ...
        answer_script = AnswerScript(
            ...
            subject=subject,
            topic=topic,
            image_data=image_base64, # Thumbnail only
            page_count=page_count,
            ocr_text=ocr_text,
            exam_date=exam_date
        )
...
async def get_evaluations():
    ...
    try:
        # Fetch evaluations (no large image fields to exclude now)
...
async def get_evaluation_full(evaluation_id: str):
    """Get full evaluation details including script preview"""
    try:
        evaluation = await db.evaluations.find_one({"id": evaluation_id}, {"_id": 0})
        if not evaluation:
            raise HTTPException(status_code=404, detail="Evaluation not found")
        
        # Join with AnswerScript to get page count and preview image
        script = await db.answer_scripts.find_one(
            {"id": evaluation['answer_script_id']}, 
            {"_id": 0, "page_count": 1, "image_data": 1}
        )
        
        # Add page info for display
        if script:
            evaluation['page_count'] = script.get('page_count', 1)
            evaluation['image_data'] = script.get('image_data')
        else:
            evaluation['page_count'] = 1
            evaluation['image_data'] = None
...
# CORS: default to the local frontend. A wildcard "*" cannot be combined with
# credentials (browsers reject it), so only enable credentials for explicit origins.
...
# DB connection is closed by the lifespan handler above (no duplicate shutdown hook).
