...
# Load .env explicitly (this module may be imported before server.py calls load_dotenv)
load_dotenv(Path(__file__).parent / ".env")

# JWT signing secret — MUST come from the environment. Never hard-code it.
SECRET_KEY = os.environ.get("SECRET_KEY", "").strip()
if not SECRET_KEY:
    # No secret configured: use a random per-process secret so we never ship a known key.
    # Tokens won't survive a restart — set SECRET_KEY in .env to make sessions persistent.
    SECRET_KEY = secrets.token_hex(32)
    logger.warning(
        "SECRET_KEY not set — using a random ephemeral secret. "
        "All issued tokens will be invalidated on restart. Set SECRET_KEY in backend/.env."
    )
